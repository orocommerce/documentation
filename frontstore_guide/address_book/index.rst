.. _frontstore-guide--address:

Address Book
============

The following information covers the Address Book section of the Account menu in OroCommerce storefront.

.. _frontstore-guide--address-overview:

.. begin

.. contents:: :local:
   :depth: 2

To locate Address Book:

1. Click **Account** in the :ref:`user menu <frontstore-guide--navigation-user-menu>` on the top left of the page.
2. Under **Account**, Click **Address Book**.

Address Book has two sections:

1. All Company Addresses
2. All My Addresses

.. image:: /frontstore_guide/img/profile/MyProfileAddressBooks.png

.. _frontstore-guide--company-address:

Company Addresses
^^^^^^^^^^^^^^^^^

.. include:: /frontstore_guide/profile/index.rst
   :start-after: begin_company_addresses
   :end-before: finish_company_addresses

My Addresses
^^^^^^^^^^^^

.. include:: /frontstore_guide/profile/index.rst
   :start-after: begin_my_addresses
   :end-before: finish_my_addresses

.. include:: /frontstore_guide/related_topics.rst
   :start-after: begin

.. include:: /user_guide/include_images.rst
   :start-after: begin


.. finish

